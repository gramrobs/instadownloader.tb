    ...
    try {
        ...
        if (blackList.includes(username)) {
            return new Promise(function (resolve, reject) {
                ...
                resolve(['Sorry, this tool does not support BlackPink, please find another wise https://www.dcard.tw/f/entertainer/p/229335287']);
            });
        }
        let score = 0;
        ...
        if (await page.$(usernameSelector)) {
            ...
            if (currentPage.search(/\/challenge\//) !== -1) {
                await page.close();
                return new Promise(function (resolve, reject) {
                    imgUrls.push(`Please verify your account again QQ`);
                    resolve(imgUrls);
                });
            }
        }
        if (await page.$(privateAccSelector)) {
            ...
            return new Promise(function (resolve, reject) {
                let timestamp = Date.now();
                let cacheArr = [];
                cacheArr[url] = `@${username} is a private account`;
                CACHE.set(homeUrl, {
                    'time': timestamp,
                    'data': cacheArr
                });

                imgUrls.push(`@${username} is a private account`);
                resolve(imgUrls);
            });
        }
        ...
        try {
            ...
            return new Promise(function (resolve, reject) {
                ...
                let timestamp = Date.now();
                let cacheArr = [];
                cacheArr[url] = `@${username} There are currently no time-limited updates`;
                CACHE.set(homeUrl, {
                    'time': timestamp,
                    'data': cacheArr
                });
                imgUrls.push(`@${username} There are currently no time-limited updates`);
                resolve(imgUrls);
            });
        }
        ...
        for (let index = 0; index < count; index++) {
            ...
            if (result == null) {
                result = `${homeUrl} Limited time download error, please try again later`;
        ...
        await page.close();
        return new Promise(function (resolve, reject) {
            resolve([`${homeUrl} An error occurred, please try again`]);
        });
    }
}
...
async function igUrl(url) {
    ...
    try {
        ...
        if (imgUrls.length < count) {
            imgUrls.push(`[caveat] ${url} Suspected download is incomplete, please try again`);
        }
        ...
        console.log(error);
        return new Promise(function (resolve, reject) {
            resolve([`${url} An error occurred, please try again`]);
        });
    }
}
...
async function twitterUrl(url) {
    try {
        ...
        if (img.length !== 0) {
            ...
            const html = await page.content();
            console.log(html);
            imgUrls.push(`${url} Can't find picture`);
        }
